...
## 设计原则与价值观 (Design Principles & Values)

QuantAda 在工程上坚持“生存优先、复杂度克制”的路线，核心原则如下：

- **高自愈 (Self-Healing First)**：网络抖动、拒单、断连、隔夜残单等异常场景优先保障“可恢复”，避免单点故障导致系统停摆。
- **无状态优先 (Stateless by Default)**：以券商真实状态为唯一事实来源，尽量不保留跨 K 历史下单意图，降低重启漂移与状态污染风险。
- **最小变更 (Minimal Changes)**：默认采用最小可行修复与最短路径实现，优先减少状态面和分支复杂度，避免框架膨胀。
- **纪律执行 (Execution Discipline)**：坚持统一的执行语义（如先卖后买、拒单当场降级重提、失败即告警），将系统行为保持在可预期边界内。

这些原则的目标不是“功能最多”，而是让策略在真实市场环境中具备更强的长期可运行性与可维护性。

### AGENTS 协作规则（简要）

本仓库已在根目录提供硬约束文件 `AGENTS.md`，用于规范 AI/Agent 协作开发。  
当 README 与协作细节存在差异时，以 `AGENTS.md` 为准。

- 协作开发默认遵循：高自愈、无状态、最小变更、纪律执行。
- 快速生成代码时，优先读取 `vibe_coding_prompts/*`，再对照对应 `base` 接口（如 `base_broker.py`）。
- 禁止重引入 `deferred/buffered` 等历史队列式状态机设计。
...
- **策略与引擎解耦 (Adapter Pattern)**
  ...
  - **本地框架回测**：可显式指定本地数据 (`CSV`/`DB`) 进行离线高效策略验证（默认不自动读取 CSV 缓存）。


- **模块化与工程分离 (SDK Mode)**
  - **数据层**：内置多源数据管理 (Tushare/AkShare)，支持责任链更新与自动缓存（CSV 缓存需显式 `--data_source csv` 才会读取）。
...
- **启发式并行优化器 (Bayesian Optimizer)**
  ...
  - **多臂赌博机**：``--metric`` 支持传入多个指标，合并展示、配合 ``--params`` 对比基准与多指标参数结果，便于人类&AI决策，实现轻量级的人机协同监督多臂赌博机。
...
- **回测 MCP 服务 (Backtest MCP Server)**
  - 提供独立包 `mcp_backtest_server`，以 MCP/stdio 对外暴露回测工具，不耦合框架代码。
  - 示例：
    ```bash
    set MCP_BACKTEST_CMD_JSON=["python","E:\Lin\Github\QuantAda\run.py"]
    set MCP_BACKTEST_WORKDIR=E:\Lin\Github\QuantAda
