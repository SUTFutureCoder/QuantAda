...
try:
    from gm.api import history, order_target_percent, get_cash, OrderType_Market, MODE_BACKTEST, \
...
class GmDataProvider(BaseLiveDataProvider):
    ...
    def get_history(self, symbol: str, start_date: str, end_date: str) -> pd.DataFrame:
        ...
        df = history(symbol=symbol, frequency='1d', start_time=start_date, end_time=end_date, fields='open,high,low,close,volume,eob', df=True)
...
class GmBrokerAdapter(BaseLiveBroker):
    ...
    def extract_run_config(context) -> dict:
        ...
        if MODE_BACKTEST is None: return {}

        if hasattr(context, 'mode') and context.mode == MODE_BACKTEST:
    ...
    def order_target_percent(self, data, target) -> GmOrderProxy:
        """
        【核心改动】下单，并返回一个包装好的 GmOrderProxy 实例。
        """
        ...
        platform_order = order_target_percent(symbol=symbol, percent=target, order_type=OrderType_Market, position_side=1)
        # 2. 将原生订单对象包装成我们自己的代理并返回
        return GmOrderProxy(platform_order[-1])
