...
class MysqlExtraProvider:
    ...
    def query(self, query, params=None, index_col=None, parse_dates=None) -> pd.DataFrame:
        try:
            df = pd.read_sql(query, self.engine, params=params, index_col=index_col, parse_dates=parse_dates)
            if df.empty:
                return None
            return df
